...
public class RfqService {
	...
	public RfqDetail getRfqOfPlant(int partId, String token) throws Exception {
		log.info("Inside GetRfqDeatilsoPlant of RfqService");
	...
	public List<Supplier> getPotentialVendorsOfRfq(int rfqId, String token) throws Exception {
		...
		if (authResponse.isValid()) {
			...
			if (rfq == null)
				//log.error("rfq id not found");
				throw new Exception("rfq id not found");
			List<Supplier> supplierList = supplierClient.getSupplier(rfq.getPartId());
...
/*
 * public List<Supplier> getPotentialVendorsOfRfq(Long rfqId) { List<RfqDetail>
 * rfqDetailList = rfqDetailRepo.getRfqDetailsByRfqId(rfqId); List<Supplier>
 * supplierList = new ArrayList<>(); Long supplierId; Supplier supplier; for
 * (RfqDetail detail : rfqDetailList) { supplierId = detail.getSupplierId();
 * supplier = supplierClient.getSupplier(supplierId); if (supplier.getFeedback()
 * >= 7) supplierList.add(supplier); } return supplierList; }
 */
